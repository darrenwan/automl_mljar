...
from src.missing.visualization import (
    plot_missing_umap, plot_missing_patterns_msno,
    sns_feat_miss_overview_row_cols, sns_feat_miss_overview, feat_distribution, sns_multicenter_missing_top,
    sns_multicenter_missing_cluster
)
from src.missing.missing_analysis import (
    compute_missing_summary,
    tableone_analysis,
    evidently_wasserstein_psi, norm_analysis, run_mcar_test
)
...
def main():
    ...
    df_xmiss.to_excel(config.DATA_WIDE_FILE.replace(".xlsx", "_clean_no_miss.xlsx"), index=False)

    sns_multicenter_missing_top(df, out_dir, center_col=config.center, top_n=10, min_missing=0.05, suffix='clean')
    sns_multicenter_missing_cluster(df, out_dir, center_col=config.center, suffix='clean')
    ...
    miss_small, miss_big = compute_missing_summary(df, features=cat_cols + num_cols, output_dir=out_dir)
    ...
    center_dep_vars, non_center_dep_vars = run_multicenter_missing_analysis(df, miss_big, output_dir=out_dir)
    ...
    multicenter_mcar_chi2_main(df, center_col=config.center, target_col=config.label,
                               features=miss_big, output_dir=out_dir)
    category_rate = len(cat_cols) / (len(num_cols) + len(cat_cols))
    if category_rate < 0.4:
        mcar = run_mcar_test(df, output_dir=out_dir)  # 类别变量>40%不适合，建议使用chi2检验
        print("mcar: ", mcar)
    plot_missing_patterns_msno(df, center_dep_vars, non_center_dep_vars, output_dir=out_dir)
    ...
    evidently_wasserstein_psi(df, config.center, cat_cols, num_cols, output_dir=out_distri_dir)
