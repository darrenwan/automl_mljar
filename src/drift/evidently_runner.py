...
from evidently.legacy.calculations.stattests import wasserstein_stat_test, psi_stat_test
...
options = DataDriftOptions(cat_features_stattest="psi", num_features_stattest="wasserstein")
...
def run_evidently(
    ...
    reference = df[
        df[center_col] == ref_center
    ]

    current = df[
        df[center_col] == cur_center
    ]
    ...
    for col in num_features:
        ...
        metrics.append(
            ColumnDriftMetric(
                column_name=col,
                stattest=wasserstein_stat_test
            )
        )
