...
# ========== 7. 综合判断逻辑 ==========
def judge_center_effect(data, missing_col='missing', center_col='center',
    ...
    formula = f'{missing_col} ~ C({center_col}, Treatment(reference="{hos_ref}")) + ' + ' + '.join(covariates)
    try:
        ...
        try:
            ...
            # 仅基于卡方检验做判断
            if p_chi2 < 0.05:
                conclusion = "存在显著的中心效应（基于卡方检验），但logistic回归模型拟合失败"
            else:
                conclusion = "未发现显著的中心效应（基于卡方检验），但logistic回归模型拟合失败"
    ...
    try:
        model_no_center = smf.logit(formula_no_center, data=data).fit(method='bfgs', maxiter=100, disp=0)
    except Exception as e:
        print(f"Warning: Could not fit reduced model due to convergence issues: {e}")
        try:
            model_no_center = smf.logit(formula_no_center, data=data).fit(method='nm', maxiter=500, disp=0)
        except:
            # 如果拟合失败，使用卡方检验结果
            evidence['lr_p'] = np.nan
            if p_chi2 < 0.05:
...
def run_multicenter_missing_analysis(df, miss_big, output_dir):
    ...
    for var in miss_big:
        ...
        conclusion, evidence, center_effects = judge_center_effect(
            df_test, missing_col='missing', center_col=config.center, covariates=[LABEL_COL, config.gender])
