...
def auc_cm(automl, X_test, y_test, plot_curves=True, output_dir=None, suffix=None, pos_label=None):
    # 外层评估
    ...
    if plot_curves:
        ...
        sns.lineplot(data=roc_data, x='FPR', y='TPR', ax=axes[0], linewidth=2, label=f'ROC curve (AUC = {auc:.4f})')
        ...
        sns.lineplot(data=pr_data, x='Recall', y='Precision', ax=axes[1], linewidth=2,
...
def train_explain_custom(df: pd.DataFrame, target_col, center_col, mode, results_path, suffix="",
                         pos_label="pneumonia", sampling_strategy=0.5):
    ...
    for hos in df[center_col].unique():
        ...
        X_train_resampled, y_train_resampled = rus.fit_resample(X_hos_train, y_hos_train)

        print(f"原始训练集形状: {X_hos_train.shape}, 比例: {y_hos_train.value_counts().to_dict()}")
        print(f"下采样后形状: {X_train_resampled.shape}, 比例: {y_train_resampled.value_counts().to_dict()}")
    ...
    scores = auc_cm(automl, X_train, y_train, plot_curves=True, output_dir=results_path,
                    suffix=suffix_train, pos_label=pos_label)
    with open(results_path + f"/scores_{suffix_train}.json", "w") as f:
        json.dump(scores, f, indent=4, ensure_ascii=False)

    suffix_test = suffix + "_test_inner"
    scores = auc_cm(automl, X_test, y_test, plot_curves=True, output_dir=results_path, suffix=suffix_test,
                    pos_label=pos_label)
    with open(results_path + f"/scores_{suffix_test}.json", "w") as f:
        json.dump(scores, f, indent=4, ensure_ascii=False)


def model_predict(test_df: pd.DataFrame, target_col: str, other_cols: list[str], results_path: str, suffix: str,
                  pos_label="pneumonia"):
    ...
    scores = auc_cm(loaded_automl, X_new, y_new, plot_curves=True, output_dir=results_path, suffix=suffix_test,
                    pos_label=pos_label)
    with open(results_path + f"/scores_{suffix_test}.json", "w") as f:
        json.dump(scores, f, indent=4, ensure_ascii=False)
...
def feats_select(dropped_feats_file):
    with open(dropped_feats_file, 'r', encoding='utf-8') as file:
...
def model_predict_out2xlsx(test_df: pd.DataFrame, target_col: str, other_cols: list[str], results_path: str,
                           suffix: str, ):
    # 假设这是你今天新获取到的测试数据，需要做预测
    X_new = test_df.drop([target_col] + other_cols, axis=1)
    ...
    test_df.sort_values(by=[config.center, "pred_proba"], ascending=[True, False], inplace=True)
